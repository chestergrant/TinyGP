  ...
  void evolve() {
    ...
    for ( gen = 1; gen < GENERATIONS; gen ++ ) {
      ...
      if ( rd.nextDouble() < CROSSOVER_PROB  ) {
		...
		int max = 0;
		int parent1_len = traverse(pop[parent1],0);
		int parent2_len = traverse(pop[parent2],0);
		max = parent2_len;
		if(parent1_len > parent2_len){
			max = parent1_len;
		}
		max += 10;
		while (traverse(newind,0) > max ){
			parent1 = tournament( fitness, TSIZE );
			parent2 = tournament( fitness, TSIZE );
			newind = crossover( pop[parent1],pop[parent2] );
			max = 0;
			parent1_len = traverse(pop[parent1],0);
			parent2_len = traverse(pop[parent2],0);
			max = parent2_len;
			if(parent1_len > parent2_len){
				max = parent1_len;
			}
